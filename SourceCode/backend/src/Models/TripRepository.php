...
class TripRepository
{
    ...
    /**
     * Khôi phục số ghế khi hủy vé (unlock ghế).
     */
    public function restoreAvailableSeats(int $tripId, int $seatsToRestore): bool
    {
        $stmt = $this->pdo->prepare(
            'UPDATE trips 
             SET available_seats = available_seats + :seats_to_restore,
                 updated_at = NOW()
             WHERE id = :id'
        );
        ...
        return $stmt->execute([
            ':id' => $tripId,
            ':seats_to_restore' => $seatsToRestore,
        ]);
    }
}
