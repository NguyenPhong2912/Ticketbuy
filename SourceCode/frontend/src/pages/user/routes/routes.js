...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Store current filter date in a variable accessible to renderRoutes
    ...
    const renderRoutes = (routes, limit = 10) => {
        ...
        const cards = limitedRoutes.map((route, index) => {
            ...
            // Lấy ngày đã lọc từ biến hoặc input (nếu có)
            const filterDate = currentFilterDate || filterDateInput?.value || '';
            let bookingUrl = `../booking/index.html?route_id=${route.id}`;
            
            // Chỉ thêm date param nếu có giá trị
            if (filterDate && filterDate.trim() !== '') {
                bookingUrl += `&date=${encodeURIComponent(filterDate)}`;
            }
            
            console.log(`Route ${route.id} booking URL:`, bookingUrl, 'filterDate:', filterDate);
            ...
            return `
                <article class="route-card">
                    ...
                    <div class="card-body">
                        ...
                        <a href="${bookingUrl}" class="btn-book">Đặt vé</a>
                    </div>
                </article>
            `;
        });
    ...
    if (dateFilterForm) {
        dateFilterForm.addEventListener('submit', (e) => {
            ...
            if (selectedDate) {
                // Update currentFilterDate immediately
                currentFilterDate = selectedDate;
                console.log('Filter date set to:', currentFilterDate);
    ...
    if (clearFilterBtn) {
        clearFilterBtn.addEventListener('click', () => {
            ...
            currentFilterDate = ''; // Clear the stored filter date
            setFilterStatus('');
            fetchRoutes();
        });
    }
